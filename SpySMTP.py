...
def main(url):
	resp = False
	try:
		text = '['+Fore.CYAN+'Scaning'+Style.RESET_ALL+']:'+url
		...
		if resp:
			...
			if getsmtp:
				text += '\t['+Fore.GREEN+'Status'+Style.RESET_ALL+']'+Fore.GREEN+' {+} SMTP '+Style.RESET_ALL
			else:
				text += '\t['+Fore.RED+'Status'+Style.RESET_ALL+']'+Fore.YELLOW+' SMTP NOT FOUND !'+Style.RESET_ALL
			if getaws:
				text += '\t['+Fore.GREEN+'Status'+Style.RESET_ALL+']'+Fore.MAGENTA+' {+} AWS '+Style.RESET_ALL
			else:
				text += '\t['+Fore.RED+'Status'+Style.RESET_ALL+']'+Fore.YELLOW+' AWS NOT FOUND !'+Style.RESET_ALL
			if getwtilio:
				text += '\t['+Fore.GREEN+'Status'+Style.RESET_ALL+']'+Fore.CYAN+' {+} TWILIO '+Style.RESET_ALL
			else:
				text += '\t['+Fore.RED+'Status'+Style.RESET_ALL+']'+Fore.YELLOW+' TWILIO NOT FOUND !'+Style.RESET_ALL
			if smsapi:
				text += '\t['+Fore.GREEN+'Status'+Style.RESET_ALL+']'+Fore.MAGENTA+' {+} SMS API '+Style.RESET_ALL
			else:
				text += '\t['+Fore.RED+'Status'+Style.RESET_ALL+']'+Fore.YELLOW+' SMS API NOT FOUND !'+Style.RESET_ALL
			if getpp:
				text += '\t['+Fore.GREEN+'Status'+Style.RESET_ALL+']'+Fore.BLUE+' {+} PAYPAL '+Style.RESET_ALL
			else:
				text += '\t['+Fore.RED+'Status'+Style.RESET_ALL+']'+Fore.YELLOW+' PAYPAL NOT FOUND !'+Style.RESET_ALL
		else:
			text = '['+Fore.RED+'Status'+Style.RESET_ALL+'] '+Fore.RED+'Fail To Crack'+Style.RESET_ALL
		...
		text = '['+Fore.CYAN+'Scaning'+Style.RESET_ALL+']:'+url
		text += Fore.RED+' Bad Unvulnrable Website/IP '
		save = open('Results/not_vulnerable.txt','a')
		asu = str(url).replace('\r', '')
		save.write(asu+'\n')
		save.close()
...
def banner():
    print('''
        {0}   _____              _____ __  __ _______ _____  
        {0}  / ____|            / ____|  \/  |__   __|  __ \ 
        {0} | (___  _ __  _   _| (___ | \  / |  | |  | |__) |
        {0}  \___ \| '_ \| | | |\___ \| |\/| |  | |  |  ___/ 
        {1}  ____) | |_) | |_| |____) | |  | |  | |  | |     
        {1} |_____/| .__/ \__, |_____/|_|  |_|  |_|  |_|     Cracker V1.0
        {1}        | |     __/ |                             
        {1}        |_|    |___/  channel : t.me/spydev_channel                           
           {2}\\━━━━━━━┯━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━/
           \t├╼ {3}BY : spydev{2}
           \t├╼ {0}Latest Mixed RCE + 33 Exploits / 256 Paths Ready For Scan
...
if __name__ == '__main__':
    ...
    try:
        ...
        tanya = raw_input("\t [>] Do you Want to continue session ? [Y/n] : ")
        ...
        try:
            ...
            try:
                lists = raw_input("\t ["+Fore.RED+">"+Style.RESET_ALL+"] List IPs/Domain ("+Fore.RED+"Ex: List.txt"+Style.RESET_ALL+") : ")
                readsplit = open(lists).read().splitlines()
            except:
                print("\t [ List Not Found ! ]")
                exit()
            try:
                numthread = raw_input("\t ["+Fore.RED+">"+Style.RESET_ALL+"] Thread ("+Fore.RED+"Recommended : 200"+Style.RESET_ALL+") : ")
            except:
